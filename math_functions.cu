...
__global__ void ReLUForward2(const int n, const Dtype* in, Dtype* out) {
...
void caffe_gpu_gemm<float>(const CBLAS_TRANSPOSE TransA,
  ...
  if (myflag)
  {
    ReLUForward2<float> << <CAFFE_GET_BLOCKS(M*N), CAFFE_CUDA_NUM_THREADS >> >(
      M*N, C, C);
...
void caffe_gpu_gemm<double>(const CBLAS_TRANSPOSE TransA,
  ...
  if (myflag)
  {
    ReLUForward2<double> << <CAFFE_GET_BLOCKS(M*N), CAFFE_CUDA_NUM_THREADS >> >(
      M*N, C, C);
    CUDA_POST_KERNEL_CHECK;
  }
}
